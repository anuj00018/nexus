...
-- ===================================================================
-- TABLE: event_ratings
-- Post-event 5-star ratings and written reviews
-- ===================================================================
...
-- Public can read all event ratings (needed for calculation and review lists)
create policy "Event ratings are viewable by everyone"
  on public.event_ratings for select
  using (true);

-- Anyone can submit their own event rating
create policy "Users submit own event ratings"
  on public.event_ratings for insert
  with check (auth.uid() = user_id);

-- Users can update their own event ratings
create policy "Users update own event ratings"
  on public.event_ratings for update
  using (auth.uid() = user_id);

-- Only founders/admins can delete event ratings or toggle read
create policy "Admins manage event ratings"
  on public.event_ratings for all
  using (
    exists (
      select 1 from public.users
      where id = auth.uid() and role in ('admin', 'founder')
